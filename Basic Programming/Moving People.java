...
int preRow=0,sufRow=0,preCol=0,sufCol=0;
int cumx=0,cumy=0;
...
cumy+=y;
if(cumy>0){
preRow=Math.max(preRow,Math.abs(cumy));
}
else{
sufRow=Math.max(sufRow,Math.abs(cumy));
}
if(cumx>0){
preCol=Math.max(preCol,Math.abs(cumx));
}
else{
sufCol=Math.max(sufCol,Math.abs(cumx));
}
// out.println(preRow+" "+sufRow+" "+preCol+" "+sufCol);
}
else{
if(preRow+sufRow>n || preCol+sufCol>m){
out.println(0);
}
else{
int sr=preRow,er=n-1-sufRow,scol=preCol,ecol=m-1-sufCol;
out.println(dp[er+1][ecol+1]-dp[sr][ecol+1]-dp[er+1][scol]+dp[sr][scol]);
}
}
}
out.close();
